        ...
        <el-table-column label="操作" width="150">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              type="danger"
              icon="el-icon-delete"
              @click="removeContestGame(scope.row.id)"
            ></el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    async getContestList() {
      // 根据分页获取对应的商品列表
      const res = await getContestGameList(this.queryInfo)
      if (res.status !== 200) {
        return this.$message.error('获取比赛列表失败')
      }
      console.log(res.data)
      this.$message.success('获取比赛列表成功')
    ...
    // 添加 编辑
    openDialog(contestGame = {}) {
      this.editContestGameForm = contestGame
      this.$nextTick(() => {
    ...
    async addData(editForm) {
      const res = await addContestGame(editForm)
      if (res.status !== 200) {
    ...
    async changeData(editForm) {
      ...
      const res = await updateContestGame(editForm)
      if (res.status !== 200) {
