<template>
  <div>
    ...
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item>武当论区</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- 卡片视图区域 -->
    <el-card>
      <el-row :gutter="20">
        ...
        <el-table-column label="操作" width="180">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              type="danger"
              icon="el-icon-delete"
              @click="removeForumWudang(scope.row.id)"
            ></el-button>
            <el-button
...
import {
  ...
  addForumWudang,
  updateForumWudang
} from '@/api/forumWudang'
import {
  addForumWudangreply,
  updateForumWudangreply,
  findAllReply
...
export default {
  ...
  methods: {
    async getForumList() {
      // 根据分页获取对应的商品列表
      const res = await getForumWudangList(this.queryInfo)
      if (res.status !== 200) {
        return this.$message.error('获取武当论坛列表失败')
      }
      console.log(res.data)
      this.$message.success('获取武当论坛列表成功')
    ...
    // 添加 编辑
    openDialog(forumWudang = {}) {
      this.editForumWudangForm = forumWudang
      console.log(forumWudang)
      // 当有replyId的时候，就进行回复的编辑
      if (forumWudang.replyId) {
        this.$nextTick(() => {
    ...
    async addData(editForm) {
      ...
      if (editForm.replyId) {
        // 添加回复
        const res = await addForumWudangreply(editForm)
        if (res.status !== 200) {
        ...
        // 添加主题
        const res = await addForumWudang(editForm)
        if (res.status !== 200) {
          return this.$message.error('添加失败')
        }
        this.$message.success('添加成功')
    ...
    async changeData(editForm) {
      ...
      if (editForm.replyId) {
        // 修改回复
        const res = await updateForumWudangreply(editForm)
        if (res.status !== 200) {
        ...
        // 修改主题
        const res = await updateForumWudang(editForm)
        if (res.status !== 200) {
          return this.$message.error('修改失败')
        }
        this.$message.success('修改成功')
