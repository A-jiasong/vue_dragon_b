        ...
        <el-table-column label="操作" width="180">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              type="danger"
              icon="el-icon-delete"
              @click="removeMallWeapon(scope.row.id)"
            ></el-button>
            <el-button
              size="mini"
              type="success"
...
import { addMallCart } from '@/api/mallCart'
...
export default {
  ...
  methods: {
    async getMallList() {
      // 根据分页获取对应的商品列表
      const res = await getMallWeaponList(this.queryInfo)
      if (res.status !== 200) {
        return this.$message.error('获取器械列表失败')
      }
      console.log(res.data)
      this.$message.success('获取器械列表成功')
    ...
    // 添加 编辑
    openDialog(mallWeapon = {}) {
      this.editMallWeaponForm = mallWeapon
      this.$nextTick(() => {
    ...
    async addData(editForm) {
      const res = await addMallWeapon(editForm)
      if (res.status !== 200) {
    ...
    async changeData(editForm) {
      ...
      const res = await updateMallWeapon(editForm)
      if (res.status !== 200) {
